...
### 接入第三方提供商时开放更多档位 —— 以 MiniMax M2.7 为例

DSH 接入 MiniMax 等第三方提供商时，提供商会暴露更丰富的推理档位目录，插件自动适配，把模型开放的所有思考强度等级都展开在同一条带档位吸附的滑条上。
...
- **1.2.0** — 自定义提供商思考强度支持：当上游适配器未暴露推理档位目录时，自动使用内置兜底档位（low/medium/high/max/xhigh）；README 全面翻新（徽章墙、TOC、FAQ、兼容性表、Changelog、参与贡献）；默认语言改为中文（英文版移至 README_EN.md）；截图说明修正；安装提示词优化（沙箱权限指引、镜像兜底）；新增效果图章节。
